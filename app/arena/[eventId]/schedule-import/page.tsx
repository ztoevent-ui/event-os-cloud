...
import { parseScheduleText, matchesToDbRows, MATCH_TYPE_MAP, type ParsedMatch } from '@/lib/pdf-schedule-parser';
...
async function extractPdfText(file: File): Promise<string> {
  ...
  // Use bundled worker via CDN to avoid build complexity
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    // Sort items by Y then X to preserve grid reading order
    ...
    for (const item of items) {
      ...
      if (Math.abs(y - lastY) > 5) fullText += '\n';
      fullText += item.str + ' ';
      lastY = y;
    }
    fullText += '\n--- PAGE BREAK ---\n';
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <span className="text-[9px] text-zinc-700 ml-auto font-mono">
              {match.time_slot || 'TBD'}
            </span>
          </div>
...
export default function ScheduleImportPage() {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setRawText(text);
      const matches = parseScheduleText(text);
      setParsedMatches(matches);
      setSelectedIds(new Set(matches.map(m => m.id)));
      setStep('preview');
      if (matches.length === 0) setParseError('未能在 PDF 中识别到任何赛程。请检查 PDF 格式，或使用手动录入模式。');
    } catch (e: any) {
      setParseError('PDF 解析失败：' + (e?.message || '未知错误'));
    }
    setParsing(false);
  }, []);
  ...
  const handleImport = async () => {
    ...
    const { error } = await supabase.from('arena_matches').insert(rows);
    if (error) {
      setParseError('数据库写入失败：' + error.message);
      setImporting(false);
      return;
    }
        ...
        <header className="border-b border-white/5 bg-black/30 backdrop-blur-sm px-6 py-3 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-white">PDF 赛程解析器</span>
            </div>
          </div>
          {step === 'preview' && (
            <div className="flex items-center gap-2 text-[10px] font-black text-zinc-500 uppercase tracking-widest">
              <i className="fa-solid fa-circle-check text-[#a3e635]" />
              识别到 {parsedMatches.length} 场赛事
            </div>
          )}
        </header>
        ...
        <AnimatePresence>
          {parseError && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              ...
              <button onClick={() => setParseError('')} className="hover:text-white">✕</button>
            </motion.div>
          )}
        </AnimatePresence>
            ...
            <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div
                ...
                className={`relative border-2 border-dashed rounded-2xl p-16 text-center cursor-pointer transition-all duration-300 ${
                  dragging
                    ? 'border-[#a3e635] bg-[#a3e635]/5 shadow-[0_0_40px_rgba(163,230,53,0.15)]'
              ...
              >
                ...
                {parsing ? (
                  <div className="space-y-4">
                    <i className="fa-solid fa-spinner fa-spin text-4xl text-[#0056B3]" />
                    <p className="text-sm font-black text-zinc-400 uppercase tracking-widest">正在解析 PDF...</p>
                    <p className="text-xs text-zinc-600">识别场地矩阵 · 提取氏族名称 · 匹配组别代码</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-600">支持格式：时间段 × 场地 矩阵布局</p>
                    </div>
              ...
              <div className="mt-8 p-6 bg-white/[0.02] border border-white/5 rounded-2xl">
                <div className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-4">
                  <i className="fa-solid fa-circle-info text-[#0056B3] mr-2" />识别能力
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { icon: 'fa-clock', label: '时间段', eg: '1500-1815, 1830-2100' },
                    { icon: 'fa-location-dot', label: '场地号', eg: 'Court 1 ~ Court 7' },
                    { icon: 'fa-people-group', label: '氏族名称', eg: '陈氏、林氏、黄氏...' },
                    { icon: 'fa-trophy', label: '组别代码', eg: 'MD1 MD2 WD XD V' },
                  ].map(item => (
                    <div key={item.label} className="flex items-start gap-3 p-3 bg-black/30 rounded-xl">
            ...
            <motion.div key="preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="shrink-0 border-b border-white/5 bg-black/20 px-6 py-3 flex items-center gap-4 flex-wrap">
                ...
                <select value={filterType} onChange={e => setFilterType(e.target.value)}
                  className="bg-black/50 border border-white/10 rounded-lg px-3 py-1.5 text-[10px] font-black text-white uppercase tracking-widest focus:outline-none focus:border-[#0056B3]/50">
                ...
                <div className="ml-auto flex items-center gap-3">
                  <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">
                    已选 {selectedIds.size} / {parsedMatches.length}
                  </span>
                  <button onClick={() => { setStep('upload'); setParsedMatches([]); }}
                    className="px-4 py-2 border border-white/10 rounded-lg text-[10px] font-black text-zinc-500 hover:text-white uppercase tracking-widest transition-all">
                    重新上传
                  </button>
                  <button onClick={handleImport} disabled={importing || selectedIds.size === 0}
                    ...
                    {importing
                      ? <><i className="fa-solid fa-spinner fa-spin" /> 写入中...</>
                      : <><i className="fa-solid fa-database" /> 一键导入 ({selectedIds.size})</>}
                  </button>
                </div>
              </div>

              {/* Match list grouped by time slot */}
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
                {parsedMatches.length === 0 ? (
                  <div className="text-center py-24 text-zinc-600">
                    <i className="fa-solid fa-magnifying-glass text-4xl mb-4 block opacity-40" />
                    <p className="text-sm font-black uppercase tracking-widest">未识别到任何赛事</p>
                    <p className="text-xs mt-2 text-zinc-700">请确认 PDF 包含 VS 对阵信息和氏族名称</p>
                    <div className="mt-6 p-4 bg-white/[0.02] border border-white/5 rounded-xl text-left text-[10px] text-zinc-700 font-mono max-h-48 overflow-y-auto">
                      <div className="text-zinc-500 mb-2 font-sans font-black uppercase tracking-widest text-[9px]">原始文本预览：</div>
          ...
          {step === 'done' && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <p className="text-zinc-500 text-sm mb-8">
                <span className="text-white font-black text-2xl">{importCount}</span> 场赛事已写入数据库<br />
                可在 Court Scheduling 页面进行场地分配与调度
              </p>
              <div className="flex gap-3 justify-center flex-wrap">
                ...
                <button onClick={() => { setStep('upload'); setParsedMatches([]); setSelectedIds(new Set()); }}
