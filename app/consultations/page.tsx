...
export default function GlobalConsultationsPage() {
    ...
    const handleDelete = async (id: string) => {
        ...
        setActionLoading(id);
        try {
            const { error } = await supabase.from('consulting_forms').delete().eq('id', id);
            if (error) throw error;
    ...
    const handleToggleRead = async (id: string, currentStatus: string) => {
        ...
        setActionLoading(id);
        try {
            const { error } = await supabase.from('consulting_forms').update({ status: newStatus }).eq('id', id);
            if (error) throw error;
    ...
    const handleRegenerateAi = async (id: string, loveStory: string, notes: string) => {
        setActionLoading(id);
        try {
            const summary = await getAiSummaryAction(loveStory || '', notes || '');
            if (!summary) throw new Error("AI returned empty summary");

            const { error } = await supabase.from('consulting_forms').update({ ai_summary: summary }).eq('id', id);
            if (error) throw error;

            setConsultations(prev => prev.map(c => c.id === id ? { ...c, ai_summary: summary } : c));
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        {consultations.map((c: any) => (
                            <div key={c.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition-all ${c.status === 'reviewed' ? 'border-green-200 bg-green-50/10' : 'border-zinc-100'
                                }`}>
                                {/* Action Bar */}
                                <div className="bg-zinc-50 border-b border-zinc-100 px-6 py-3 flex justify-between items-center text-xs">
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => handleToggleRead(c.id, c.status)}
                                            ...
                                            {actionLoading === c.id ? <i className="fa-solid fa-spinner fa-spin"></i> :
                                    ...
                                    <div>
                                        ...
                                        >
                                            {actionLoading === c.id ? <i className="fa-solid fa-spinner fa-spin"></i> : <i className="fa-solid fa-trash"></i>}
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="bg-gradient-to-r from-amber-50 to-white p-4 rounded-lg border border-amber-100 mb-6 relative overflow-hidden group">
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <button
                                                onClick={() => handleRegenerateAi(c.id, c.love_story, c.important_notes)}
                                                disabled={actionLoading === c.id}
                                            ...
                                            >
                                                {actionLoading === c.id ? <i className="fa-solid fa-spinner fa-spin"></i> : <i className="fa-solid fa-arrows-rotate"></i>} Regenerate
                                            </button>
                                        </div>
